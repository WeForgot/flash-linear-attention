...
from fla.layers.lact import LaCT, LowRankFastWeight
...
from fla.modules import GatedMLP as LaCTMLP
...
class LaCTBlock(GradientCheckpointingLayer):

    def __init__(self, config: LaCTConfig, layer_idx: int):
        super().__init__()
...
class LaCTPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(
        ...
        if isinstance(module, LowRankFastWeight):
            if not getattr(module.w_left, '_is_hf_initialized', False):
                module._init_weights()
            return
        if isinstance(module, LaCT):
            with torch.no_grad():
                nn.init.ones_(module.qk_scale)
                nn.init.zeros_(module.qk_offset)
                if module.w0_w2_low_rank <= 0:
                    nn.init.normal_(module.w0, mean=0.0, std=1.0 / math.sqrt(module.d_in))
                    nn.init.normal_(module.w2, mean=0.0, std=1.0 / math.sqrt(module.d_in))
...
class LaCTModel(LaCTPreTrainedModel):
    ...
    def __init__(self, config: LaCTConfig):
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        if output_attentions:
            warnings.warn("`LaCTModel` does not `output_attentions` now, setting it to `False`.")
        ...
        if not return_dict:
            return tuple(
                i for i in [hidden_states, past_key_values, all_hidden_states, all_attns] if i is not None
            )
...
class LaCTForCausalLM(LaCTPreTrainedModel, FLAUnsupportedCacheGenerationMixin):
